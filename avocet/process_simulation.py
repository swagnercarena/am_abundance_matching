# TODO: 1) Grab the ids for every single halo in the catalog.

# This code was taken from Ethan Nadler's subhalo abundance matching code. 
# Modifications have been made for use with this package.
#imports
...
additional_fields_subtree = ['depth_first_id', 'desc_id', 'mmp', 'vx', 'vy', 
    'vz', 'pid']

def get_sim_data(halo_id,path_to_data,additional_fields,additional_fields_subtree):
    """
    ...
    path_to_data (string): path to consistent_trees data
    ...
    Returns:
    sim (iterable): SimulationAnalysis object
    tree (array): dictionary of main branch of host halos
    subtree (iterable): tree objects for all subhalos
    """
    sim = SimulationAnalysis(trees_dir=path_to_data)
    tree = sim.load_main_branch(halo_id, additional_fields=additional_fields)
    subtree = sim.load_tree(halo_id, additional_fields=additional_fields_subtree)
    ...
    for key in ids.keys():
        ...
        for i in range (1,len(scale[key])-1):
            ...
            for j in range (1,len(subtree_snap)-1):
                # This is met if the object is its own descendent (it stopped existing)
